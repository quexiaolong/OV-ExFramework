...
public class EventDispatcher implements IProcessEvent, ISystemEvent {
    ...
    @Override // com.vivo.services.rms.sdk.IProcessEvent
    ...
    @Override // com.vivo.services.rms.sdk.IProcessEvent
    public void startActivity(String pkgName, String processName, int pid, int uid, int isBeforeStart) {
        RMServer.getInstance().postProcessEvent(7, builderArgs(pid, uid, isBeforeStart, pkgName, processName, null));
    }

    public void startActivityOnVD(String pkgName, String processName, String className, int pid, int uid, int started) {
        RMServer.getInstance().postProcessEvent(11, builderArgs(pid, uid, started, pkgName, processName, className));
    }

    public void notifyRmsForSpecial(int type) {
        RMServer.getInstance().postProcessEvent(12, builderArgs(type, Constants.Setting.ARD9_FACE_UNLOCK_FAST_UNLOCK));
    }
    ...
    @Override // com.vivo.services.rms.sdk.ISystemEvent
    public void setAppList(String typeName, ArrayList<String> list) {
        RMServer.getInstance().postSystemEvent(0, builderArgs(typeName, list));
    }
