package android.hardware.camera.common.V1_0;

import java.util.ArrayList;

/* loaded from: classes.dex */
public final class Status {
    public static final int CAMERA_DISCONNECTED = 6;
    public static final int CAMERA_IN_USE = 2;
    public static final int ILLEGAL_ARGUMENT = 1;
    public static final int INTERNAL_ERROR = 7;
    public static final int MAX_CAMERAS_IN_USE = 3;
    public static final int METHOD_NOT_SUPPORTED = 4;
    public static final int OK = 0;
    public static final int OPERATION_NOT_SUPPORTED = 5;

    public static final String toString(int o) {
        if (o == 0) {
            return "OK";
        }
        if (o == 1) {
            return "ILLEGAL_ARGUMENT";
        }
        if (o == 2) {
            return "CAMERA_IN_USE";
        }
        if (o == 3) {
            return "MAX_CAMERAS_IN_USE";
        }
        if (o == 4) {
            return "METHOD_NOT_SUPPORTED";
        }
        if (o == 5) {
            return "OPERATION_NOT_SUPPORTED";
        }
        if (o == 6) {
            return "CAMERA_DISCONNECTED";
        }
        if (o == 7) {
            return "INTERNAL_ERROR";
        }
        return "0x" + Integer.toHexString(o);
    }

    public static final String dumpBitfield(int o) {
        ArrayList<String> list = new ArrayList<>();
        int flipped = 0;
        list.add("OK");
        if ((o & 1) == 1) {
            list.add("ILLEGAL_ARGUMENT");
            flipped = 0 | 1;
        }
        if ((o & 2) == 2) {
            list.add("CAMERA_IN_USE");
            flipped |= 2;
        }
        if ((o & 3) == 3) {
            list.add("MAX_CAMERAS_IN_USE");
            flipped |= 3;
        }
        if ((o & 4) == 4) {
            list.add("METHOD_NOT_SUPPORTED");
            flipped |= 4;
        }
        if ((o & 5) == 5) {
            list.add("OPERATION_NOT_SUPPORTED");
            flipped |= 5;
        }
        if ((o & 6) == 6) {
            list.add("CAMERA_DISCONNECTED");
            flipped |= 6;
        }
        if ((o & 7) == 7) {
            list.add("INTERNAL_ERROR");
            flipped |= 7;
        }
        if (o != flipped) {
            list.add("0x" + Integer.toHexString((~flipped) & o));
        }
        return String.join(" | ", list);
    }
}